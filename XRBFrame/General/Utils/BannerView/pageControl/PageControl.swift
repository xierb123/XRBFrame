...
class LoopPageView: UIView {
   ...
   /// 背景色，默认灰色
    ...
    private func setupAllPages(_ oldValue:Int) {
        ...
        if self.heigh > self.frame.height {
            self.heigh = self.frame.height
        }
        self.normalWidth =  self.heigh
        ...
        self.backView.frame = CGRect(x: frameX, y: (self.frame.maxY -  self.heigh) * 0.5, width: width, height: self.heigh)
         var x:CGFloat = 0
         for i in 0 ..< self.pages {
             let view = UIView()
             if i == self.currentPage {
                 view.frame = CGRect(x: x, y: 0, width: self.selectWidth, height: heigh)
                 view.backgroundColor = self.selectColor
                 x += self.selectWidth + self.pageMargin
             }else {
                view.frame = CGRect(x: x, y: 0, width: self.normalWidth, height: heigh)
                 view.backgroundColor = self.normalColor
                 x += self.normalWidth + self.pageMargin
             }
             view.layer.cornerRadius = self.heigh * 0.5
    ...
    private func setupCurrentPage(_ oldValue:Int) {
        if oldValue != self.currentPage {
            ...
            for i in 0 ..< self.pages {
                ...
                if i == self.currentPage {
                    view.frame = CGRect(x: viewX, y: 0, width: self.selectWidth, height: self.heigh)
                    viewX += self.selectWidth + self.pageMargin
                    view.backgroundColor = self.selectColor
                    continue
                }
                view.frame = CGRect(x: viewX, y: 0, width: self.normalWidth, height: self.heigh)
