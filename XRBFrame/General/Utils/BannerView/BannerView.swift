...
class BannerView: UIView {
    ...
    public var animationDatas:[String]?
    /// 图片数组
    ...
    /// 自动滚动间隔时间, 默认2s
    public var autoScrollTimeInterval:CGFloat = 5.0
    /// itme 宽度 默认为视图的宽度
...
extension BannerView {
    private func setupConfig() {
        ...
        self.placeholderImage = UIImage(named: "indexbanner")
    }
...
//        self.removeImage()
        automaticScroll()
//        self.perform(#selector(removeImage), with: nil, afterDelay: 4)
    }
...
//            self.bannerDatas.removeAll()
        ...
        for imageStr in self.imageDatas! {
            ...
            if i < animationDatas?.count ?? 0 {
                infoModel.animationUrl = animationDatas?[i]
            }
            i += 1
        }
    ...
    private func scrollCollectionItemIndexCenter() {
        ...
        if self.collectionView?.contentOffset.x == 0 && self.nums > 0 {
            ...
            self.dragIndex = 1
            playAnimation(index: targeIndex)
//            playAnimation(index: Int(targeIndex))
    ...
    private func automaticScroll() {
        ...
        self.dragIndex = 1
        AnimationStart(index: targeIndex)
    ...
    private func playAnimation(index:Int) {
       
        let cell = self.collectionView?.cellForItem(at: IndexPath(row: index, section: 0)) ??  self.collectionView?.cellForItem(at: IndexPath(row: index - 1, section: 0))
        if cell?.isKind(of: BannerViewCell.self) ?? false {
           let animationCell = cell as! BannerViewCell
//            animationCell.playAnimation()
            if self.dragIndex == 0 {
              
            }else if self.dragIndex == 1 {
                animationCell.playAnimation()
            }else if self.dragIndex == -1 {
                animationCell.AnimationEnd()
            }
       }
    }
    
    private func AnimationStart(index:Int) {
       
        let cell = self.collectionView?.cellForItem(at: IndexPath(row: index, section: 0))
        if cell?.isKind(of: BannerViewCell.self) ?? false {
           let animationCell = cell as! BannerViewCell
            animationCell.AnimationStart()
       }
    }
}
...
extension BannerView {
    ...
    func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        ...
        if self.collectionView?.visibleCells.count == 3 {
            ...
            let arr =  self.collectionView?.visibleCells.sorted(by: { (cell0, cell1) -> Bool in
                if let index0 = self.collectionView?.indexPath(for: cell0), let index1 = self.collectionView?.indexPath(for: cell1) {
                    return index0.row < index1.row
                }
                return true
            })
            
            if  let cell = arr?.first{
                if cell.isKind(of: BannerViewCell.self)  {
                   let animationCell = cell as! BannerViewCell
                    animationCell.AnimationEnd()
               }
            }
            
            if  let cell = arr?.last{
                if cell.isKind(of: BannerViewCell.self)  {
                   let animationCell = cell as! BannerViewCell
                    animationCell.AnimationStart()
               }
...
//        playAnimation(index: Int(scrolleIndex))
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        if velocity.x > 0 || page > 0 {
            self.dragIndex = 1
            //        //手动滑动时，有图片从右侧划出，不能用currentIndex，需要+1
            AnimationStart(index: self.currentIndex()+1 % self.bannerDatas.count)
            scrollToIndex(self.currentIndex()+1 % self.bannerDatas.count)
        }else if velocity.x < 0 || page < 0 {
            self.dragIndex = -1
            //        //手动左滑动时，有图片从右侧划出，不能用currentIndex，需要+1
            scrollToIndex(self.currentIndex()-1 % self.bannerDatas.count)
        }else {
             self.dragIndex = 0
        }
